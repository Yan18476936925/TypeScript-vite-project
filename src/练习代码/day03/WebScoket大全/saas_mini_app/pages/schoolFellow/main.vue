...
<script>
	...
	export default {
		...
		async onShow() {
			...
			// #endif
			if(!!this.$store.getters.openid && !!this.isLeave) {
				this.isLeave = false
				this.getUrl();
			}
		...
		async onLoad(option) {
			uni.$off("PAGE_schoolFellow_REFRESH");
			uni.$on("PAGE_schoolFellow_REFRESH", () => {
		...
		methods: {
			...
			async getUrl() {
				...
				// 链接选择（打开调试模式）
				// this.url = this.base.webViewUrl + `/h5/index?name=schoolFellow.html&minAppOpenId=${this.$store.getters.openid}&debug=true`
				// 链接选择（关闭调试模式）
				this.url = this.base.webViewUrl + `/h5/index?name=schoolFellow.html&minAppOpenId=${this.$store.getters.openid}`
				// 灰度 turntable_act
				// this.url = this.base.webViewUrl +  `/client/turntable_act/index_test?${_parameter1}&debug=true`
				global._logger.writeLogs("星球跳转,跳转链接", this.url);
